class OrderMyOrderPage {
  elements = {
    ...
    statusFilter: () =>
      cy.get('div div[class="flex w-full items-center"] div[class="mr-8"]'),
    searchStatus: () => cy.get('div input[placeholder="Search..."]'),
    status_: () =>
      cy.get(
        'div[class="flex items-center space-x-8"] div[class="px-6 py-4 text-ps leading-1 text-center relative pr-8 rounded-4"] span[class]'
      ),
  };
  statusFilter_Orders = () => {
